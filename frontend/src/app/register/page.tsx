...
import { UserPlus, Mail, Key, User, Phone, AlertCircle } from 'lucide-react';
import { motion } from 'framer-motion';
import MagneticButton from '../../components/MagneticButton';

export default function Register() {
  const { user, registerUser, login } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(`Welcome to DineFlow, ${loggedUser.fullName.split(' ')[0]}!`, 'success');
      router.push('/profile');
    } catch (err: any) {
    ...
    <div className="bg-[#0D0B08] min-h-screen flex items-center justify-center px-4 py-24 relative overflow-hidden text-white">
      ...
      <div className="absolute inset-0 z-0 opacity-10 pointer-events-none">
        <svg className="w-full h-full text-brand-gold" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
          <pattern id="register-jali" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
            <circle cx="10" cy="10" r="0.8" fill="currentColor" />
          </pattern>
          <rect width="100" height="100" fill="url(#register-jali)" />
        </svg>
      </div>
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-xs text-white/40 font-light leading-relaxed">
            Join DineFlow for easy reservation and order tracking
          </p>
        </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                <User className="absolute left-3.5 top-3.5 w-4.5 h-4.5 text-white/20" />
                <input
                  type="text"
                  required
                  value={fullName}
            ...
            <div>
              ...
              <div className="relative">
                <Phone className="absolute left-3.5 top-3.5 w-4.5 h-4.5 text-white/20" />
                <input
                  type="tel"
                  required
                  value={phone}
